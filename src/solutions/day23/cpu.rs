...
impl CPU {
    pub fn new(id: usize, inbound_msgs: Receiver<Packet>, network: Network, activity_count: Arc<AtomicU64>) -> Self {
    ...
    pub fn run(self, nic_program: Vec<i64>) {
        ...
        // (They will won't be called concurrently by the IntcodeComputer, so this is fine.)
    ...
    fn provide_cpu_input(self_: Rc<RefCell<Self>>) -> impl FnMut() -> i64 {
        ...
        move || {
            ...
            if let Some(id) = id.take() {
                ...
                // Note that we unblock before consuming the message.
                // This ensures the system doesn't reach activity_count=0 accidentally,
                // even though there's no deadlock.
                self_.unblock();
                self_.activity_count.fetch_sub(1, SeqCst); // -1 message in flight.
    ...
    fn block(&mut self) {
        ...
        if self.num_consecutive_polls == Self::INACTIVE_THRESH {
            self.activity_count.fetch_sub(1, SeqCst); // -1 active CPU (this one).
        }
    }
    ...
    fn unblock(&mut self) {
        if self.num_consecutive_polls >= Self::INACTIVE_THRESH {
            self.activity_count.fetch_add(1, SeqCst); // +1 active CPU (this one).
        }
        ...
        move |val| {
            ...
            if outbound_packet.len() == 3 {
                ...
                let dest = if 0 <= addr && addr < NUM_CPUS as i64 {
                    ...
                    self_.activity_count.fetch_add(1, SeqCst); // +1 message in flight.
